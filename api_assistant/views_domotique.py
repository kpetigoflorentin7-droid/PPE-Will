...
def _envoyer_commande(appareil: AppareilConnecte, commande: str, params: dict, user) -> bool:
    ...
    Protocoles supportés :
      ...
      - mqtt  → publication MQTT (nécessite paho-mqtt installé)
    ...
    try:
        if appareil.protocole == 'wifi' and appareil.adresse_ip:
        ...
        elif appareil.protocole == 'mqtt' and appareil.topic_mqtt:
            # Nécessite : pip install paho-mqtt
            try:
                import paho.mqtt.publish as publish
                payload = {"cmd": commande, **params}
                import json
                publish.single(
                    appareil.topic_mqtt,
                    payload=json.dumps(payload),
                    hostname="localhost",  # ← remplace par ton broker MQTT
                    port=1883,
                )
                succes = True
            except ImportError:
                # paho-mqtt non installé : on passe par Flutter (cache)
                succes = False
