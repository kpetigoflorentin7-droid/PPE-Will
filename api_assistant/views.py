...
from .models import Message, Alarme, AssistantStatus, AppareilConnecte, AppInstallee
from .views_domotique import _detecter_commande_domotique
from .views_playlist import traiter_commande_vocale_playlist
from .views_tv import _detecter_commande_tv_app

# ─── CONFIG ─────────────────────────────────────────────────────────────────
# Clé lue depuis l'environnement (voir settings.py + fichier .env) — plus
# aucune clé API en dur dans le code source.
client_gemini = genai.Client(api_key=settings.GEMINI_API_KEY)
GEMINI_MODEL  = "gemini-2.5-flash"  # rapide et peu coûteux, adapté à un assistant vocal
...
# NOTE : APPS_DATABASE (variable en mémoire) a été remplacée par le modèle
# AppInstallee (en base de données) pour survivre aux redémarrages du
# serveur et être propre à chaque utilisateur. Voir _detecter_intention()
# et sync_apps() ci-dessous.
...
def _detecter_intention(prompt: str, user) -> dict | None:
    """
    ...
    if any(mot in p for mot in mots_ouvrir):
        apps_installees = AppInstallee.objects.filter(utilisateur=user)
        for app in apps_installees:
            if app.nom in p:
                return {"action": "OPEN_APP", "package": app.package, "app_name": app.nom.capitalize(),
                        "reponse_ia": f"J'ouvre {app.nom.capitalize()} !"}
...
def _traiter_message(user, prompt: str):
    """
    Analyse le prompt, exécute l'action (domotique, alarme, appel, app…)
    ...
    try:
        ...
        # ── -1. Domotique (priorité maximale avant toute autre intention) ──
        ...
        # ── -0.5. Playlist : lecture ET arrêt (STOP_MUSIC inclus) ─────────
        elif commande_playlist := traiter_commande_vocale_playlist(prompt, user):
            reponse_ia  = commande_playlist['reponse_ia']
            action_data = commande_playlist['action_to_do']
            cache.set(f"will_pending_action_{user.id}", action_data, timeout=30)

        # ── -0.4. Ouverture d'appli streaming sur la TV (YouTube/Netflix) ──
        elif commande_tv := _detecter_commande_tv_app(prompt, user):
            reponse_ia  = commande_tv['reponse_ia']
            action_data = commande_tv['action_to_do']
            if action_data:
                cache.set(f"will_pending_action_{user.id}", action_data, timeout=30)

        # ── 0. Intentions système (apps, YouTube, Google, Maps) ──────────
        elif intention := _detecter_intention(prompt, user):
        ...
        # ── 5. Météo ─────────────────────────────────────────────────────
            ...
            ville   = "Lomé"
            m_ville = re.search(r'(?:à|a|pour|sur|de)\s+([A-ZÀ-ÿa-z\-]+)', prompt)
            if m_ville:
                ville = m_ville.group(1).strip().capitalize()

            url = (f"https://api.openweathermap.org/data/2.5/weather"
                   f"?q={ville}&appid={settings.OPENWEATHER_API_KEY}&units=metric&lang=fr")
            try:
        ...
        # ── 6. IA Gemini (conversation générale) ──────────────────────────
            ...
            appareils_str = ", ".join([
                a.nom for a in AppareilConnecte.objects.filter(utilisateur=user, est_actif=True)
            ]) or "aucun appareil connecté"
            ...
            contexte = (
                ...
                f"Les appareils connectés de l'utilisateur sont : {appareils_str}. "
                "Tu peux les contrôler si on te le demande (allumer, éteindre, régler la température, etc.)."
            )
            try:
                gemini_response = client_gemini.models.generate_content(
                    ...
                    config=types.GenerateContentConfig(
                        system_instruction=contexte,
                        max_output_tokens=200,
                    ),
                )
                reponse_ia = gemini_response.text.strip()
...
def get_weather(request):
    ...
    url  = (f"https://api.openweathermap.org/data/2.5/forecast"
            f"?q={city}&appid={settings.OPENWEATHER_API_KEY}&units=metric&lang=fr")
    try:
        r = requests.get(url, timeout=5)
        d = r.json()
        if r.status_code != 200:
...
def sync_apps(request):
    ...
    AppInstallee.objects.filter(utilisateur=request.user).delete()
    AppInstallee.objects.bulk_create([
        AppInstallee(utilisateur=request.user, nom=nom.lower(), package=package)
        for nom, package in apps.items()
    ])

    count = len(apps)
    print(f"✅ {count} apps synchronisées et enregistrées pour {request.user.username}.")
